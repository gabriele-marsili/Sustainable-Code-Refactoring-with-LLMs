function* combinations(len, items) {
    ...
    while (true) {
        ...
        if (unique.size === len) {
            yield indexes.slice(); // Yield a copy to prevent modification
        }

        let i = 0;
        indexes[0]++;

        while (i < len && indexes[i] >= items.length) {
            indexes[i] = 0;
            i++;
            if (i < len) {
                indexes[i]++;
...
function solve(puzzle) {
    ...
    for (let combination of combinations(letters.length, digits)) {
        ...
        if (sumOfTerms !== translate(sum, map)) {
            continue;
        }
        ...
        if (!firstZero) {
            if (solution === null) {
                ...
                return null; // More than one solution
            }
        }
    }
