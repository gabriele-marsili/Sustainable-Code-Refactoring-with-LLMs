...
export function preparationTime(layers, meanPrepTime) {
	const defaultPrepTime = 2;
	return layers.length * (meanPrepTime === undefined ? defaultPrepTime : meanPrepTime);
}
...
export function scaleRecipe(recipe, factor) {
	const actualFactor = (factor === undefined ? 1 : factor * 0.5);
	const scaledRecipe = {};

	for (const ingredient in recipe) {
		if (Object.hasOwn(recipe, ingredient)) {
			scaledRecipe[ingredient] = recipe[ingredient] * actualFactor;
		}
	}
