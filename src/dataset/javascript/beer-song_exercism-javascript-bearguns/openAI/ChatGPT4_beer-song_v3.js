class BeerSong {
  ...
  actionBuilder(num) {
    return num === 0 ? this.actions.nomore : num === 1 ? this.actions.onemore : this.actions.def;
  }

  howManyBottles(num) {
    return num === 0 ? "No more bottles of beer" : num === 1 ? "1 bottle of beer" : `${num} bottles of beer`;
  }

  nextVerse(num) {
    const nextAmt = num - 1;
    return (nextAmt === -1 ? this.howManyBottles(99) : this.howManyBottles(nextAmt)) + " on the wall.\n".toLowerCase();
  }
