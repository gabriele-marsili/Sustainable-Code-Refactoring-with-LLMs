...
func ToRomanNumeral(dec int) (string, error) {
	...
	result.Grow(16) // Pre-allocate reasonable capacity
	
	for _, pair := range romanToDec {
		count := dec / pair.decimal
		if count > 0 {
			result.WriteString(strings.Repeat(pair.roman, count))
			dec -= count * pair.decimal
		}
	}
