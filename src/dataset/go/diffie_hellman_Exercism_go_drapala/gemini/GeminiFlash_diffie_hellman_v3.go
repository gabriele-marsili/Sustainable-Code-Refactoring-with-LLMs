...
func PrivateKey(p *big.Int) *big.Int {
	one := big.NewInt(1)
	limit := new(big.Int).Sub(p, one) // 1 < a < p
	a, err := rand.Int(rand.Reader, limit)
	if err != nil {
		panic(err)
	}
	a.Add(a, one) // Ensure a >= 2
	return a
}
...
func PublicKey(private, p *big.Int, g int64) *big.Int {
	...
	base := big.NewInt(g)
	result := new(big.Int).Exp(base, private, p) // (g^private) % p
	return result
}

// 4. Generate a shared secret key
func SecretKey(private1, public2, p *big.Int) *big.Int {
	result := new(big.Int).Exp(public2, private1, p) // (public)^private % p
	return result
}
