...
func (s IntList) Filter(fn func(int) bool) (filtered IntList) {
	filtered = make(IntList, 0, len(s)) // Preallocate memory
	for _, v := range s {
		if fn(v) {
...
func (s IntList) Length() (length int) {
	return len(s) // Directly use len for efficiency
}

func (s IntList) Map(fn func(int) int) (mapped IntList) {
	mapped = make(IntList, len(s)) // Preallocate memory
	for i, v := range s {
		mapped[i] = fn(v)
	}
	return mapped
...
func (s IntList) Reverse() (reversed IntList) {
	reversed = make(IntList, len(s)) // Preallocate memory
	for i, v := range s {
...
func (s IntList) Append(toAppend IntList) (appended IntList) {
	return append(s, toAppend...) // No changes needed
}
...
func (s IntList) Concat(lists []IntList) (concatenated IntList) {
	...
	concatenated = make(IntList, 0, totalLen) // Preallocate memory
