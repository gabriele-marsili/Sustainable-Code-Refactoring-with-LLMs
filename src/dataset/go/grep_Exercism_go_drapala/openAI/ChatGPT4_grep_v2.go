...
func Search(pattern string, flags, files []string) []string {
	...
	for _, file := range files {
		content := fileMap[file]
		matchedLines := matchPattern(regex, pattern, file, content, printFileName, isFlagSet("-n"), isFlagSet("-i"), isFlagSet("-x"), isFlagSet("-v"))
		if isFlagSet("-l") {
			if len(matchedLines) > 0 {
				result = append(result, file)
			}
		} else {
			result = append(result, matchedLines...)
		}
	}
	return result
}

func matchPattern(regex *regexp.Regexp, pattern, fileName string, content []string, printFileName, printLine, insensitive, wholeLine, invert bool) []string {
	var result []string
	for i, line := range content {
		...
		match := (wholeLine && line == pattern) || (!wholeLine && regex.MatchString(line))
		if invert {
			match = !match
		}
