...
// Open creates a bank account with a balance and doesn't allow negative balances
...
// Deposit adds money to a bank account, doesn't allow negative balances or closed accounts
func (account *Account) Deposit(amount int64) (int64, bool) {
	account.mu.Lock()
	if !account.open || (amount < 0 && account.balance < -amount) {
		account.mu.Unlock()
		return 0, false
	}
