...
func (err rangeError) Error() string { return string(err) }
func (err syntaxError) Error() string { return string(err) }

func ParseHex(hexString string) (int64, error) {
	if len(hexString) == 0 {
		return 0, syntaxError("No digits")
	} else if len(hexString) >= 16 {
...
func HandleErrors(hexStrings []string) []string {
	...
	for i, hexString := range hexStrings {
		...
		if errors.As(err, new(rangeError)) {
			errorList[i] = "range"
		} else if errors.As(err, new(syntaxError)) {
			errorList[i] = "syntax"
		} else {
			errorList[i] = "none"
		}
