...
func search(pattern string, config configuration, files []string) []line {
	...
	for _, file := range files {
		fileMatches := searchInFile(pattern, config, file)
		matches = append(matches, fileMatches...)
	}
	return matches
}

func searchInFile(pattern string, config configuration, filename string) []line {
...
func format(matches []line, config configuration) []string {
	...
	for _, match := range matches {
		formattedMatch := formatMatch(match, config)
		if config.printFileNames {
			if !resultMap[formattedMatch] {
				resultMap[formattedMatch] = true
				result = append(result, formattedMatch)
			}
		} else {
			result = append(result, formattedMatch)
		}
