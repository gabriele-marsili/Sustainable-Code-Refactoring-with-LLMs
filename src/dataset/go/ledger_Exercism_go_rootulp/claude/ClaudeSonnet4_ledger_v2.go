...
func FormatLedger(currency string, locale string, entries []Entry) (string, error) {
	...
	output.WriteString(header(locale))
	
	sorted := sortEntries(entries)
	for i := range sorted {
		output.WriteString(formatEntry(locale, currency, sorted[i]))
	}
...
func formatChange(locale string, currency string, cents int) string {
	...
	paddedChange := fmt.Sprintf("%03d", absValue)
	...
	if cents < 0 {
		return formatNegative(locale, result)
	}
	return result
}
