...
func formatChange(locale string, currency string, cents int) string {
	...
	paddedChange := fmt.Sprintf("%03s", absCents)
	integralPart := formatIntegralPart(locale, paddedChange[:len(paddedChange)-2])
	decimalPart := paddedChange[len(paddedChange)-2:]
	change := currencyToSymbol[currency] + locales[locale].DecimalPoint + strings.Join([]string{integralPart, decimalPart}, locales[locale].DecimalPoint)
