...
// counter counts the read/write accesses and total bytes transferred.
type counter struct {
...
// newCounter creates a new counter to record I/O operations.
func newCounter(iofunc func([]byte) (int, error)) *counter {
	return &counter{iofunc: iofunc}
}

// doIoFunc performs the I/O function and updates the counters.
