...
func shiftchar(newchar rune) string {
	if newchar > 'z' { // if we fall right-side of z
		return string(newchar - 26) // wrap back left into a-z range
	} else if newchar < 'a' { // if we fall left-side of a
		return string(newchar + 26) // wrap back right into a-z range
	}
	return string(newchar) // keep as is
}
...
func (c shift) Encode(input string) string {
	...
	for _, x := range input { // for each character in clean input
		...
		sb.WriteString(shiftchar(newchar))
	}
	return sb.String()
}
