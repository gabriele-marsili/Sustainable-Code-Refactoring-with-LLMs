...
func getDigitsAndCommands(input string) ([]string, []string) {
	...
	i := 0
	for i < len(input) {
		// Skip non-digit characters to find start of number
		...
		// Extract command between numbers
		if len(digits) > 1 {
			cmdStart := len(input[:start])
			for cmdStart > 0 && (input[cmdStart-1] >= '0' && input[cmdStart-1] <= '9') {
				cmdStart--
			}
			commands = append(commands, input[cmdStart:start])
		}
	}
