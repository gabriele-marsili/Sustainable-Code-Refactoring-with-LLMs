...
func (rc *readCounter) ReadCount() (int64, int) {
	totalBytesRead := atomic.LoadInt64(&rc.totalBytesRead)
	numReads := atomic.LoadInt64(&rc.numReads)

	return totalBytesRead, int(numReads)
}
...
func (wc *writeCounter) WriteCount() (int64, int) {
	totalBytesWritten := atomic.LoadInt64(&wc.totalBytesWritten)
	numWrites := atomic.LoadInt64(&wc.numWrites)

	return totalBytesWritten, int(numWrites)
}
