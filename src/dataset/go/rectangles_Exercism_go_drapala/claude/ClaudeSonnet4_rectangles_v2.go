...
func intersection(s1, s2 []int) []int {
	...
	inter := make([]int, 0, min(len(s1), len(s2)))
	i, j := 0, 0
...
func Count(diagram []string) int {
	...
	// Sort each group for efficient intersection
	...
	for x1 := 0; x1 < width; x1++ {
		...
		for x2 := x1 + 1; x2 < width; x2++ {
			...
			// Check all pairs of intersections for valid rectangles
			for left := 0; left < len(intersections); left++ {
				for right := left + 1; right < len(intersections); right++ {
					...
					// Check if the rectangle is valid immediately
