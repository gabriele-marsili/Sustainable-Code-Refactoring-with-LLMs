...
func ConsonantClusterIndex(sentence string) int {
	for i := 0; i < len(sentence); i++ {
		...
		// Check for "xr" and "yt"
		if i < len(sentence)-1 {
			if (sentence[i] == 'x' && sentence[i+1] == 'r') ||
			   (sentence[i] == 'y' && sentence[i+1] == 't') {
				return i
			}
		}
	}
...
func PigConverter(word string) string {
	...
	// Check for 2-letter word ending in 'y'
...
func Sentence(sentence string) string {
	...
	builder.Grow(len(sentence) + len(words)*2) // Pre-allocate approximate space
