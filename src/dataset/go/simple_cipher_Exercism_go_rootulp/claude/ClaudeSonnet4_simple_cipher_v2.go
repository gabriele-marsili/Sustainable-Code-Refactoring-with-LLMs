...
func (c shift) Encode(input string) (encoded string) {
	...
	for _, r := range input {
		if r >= 'A' && r <= 'Z' {
			result.WriteByte(byte(shiftLetter(r+32, c.distance)))
		} else if r >= 'a' && r <= 'z' {
			result.WriteByte(byte(shiftLetter(r, c.distance)))
		}
	}
...
func (v vigenere) Encode(input string) (encoded string) {
	...
	for i, r := range stripped {
		distance := int(v.key[i%v.keyLen] - 'a')
		result.WriteByte(byte(shiftLetter(r, distance)))
	}
	return result.String()
}
