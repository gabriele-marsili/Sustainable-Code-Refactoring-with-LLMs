...
func groupCards(ranks *[5]int) string {
	rankGroups := make(map[int]int, 5) // Initialize with capacity 5

	for _, r := range ranks {
		rankGroups[r]++
	}

	rankList := make([][2]int, 0, len(rankGroups)) // Initialize with capacity
	...
	groupSize.Grow(5) // Pre-allocate space for up to 5 characters
