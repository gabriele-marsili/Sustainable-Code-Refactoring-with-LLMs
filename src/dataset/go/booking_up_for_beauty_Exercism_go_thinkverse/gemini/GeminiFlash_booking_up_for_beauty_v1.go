...
func AnniversaryDate() time.Time {
	now := time.Now()
	return time.Date(now.Year(), anniversaryMonth, anniversaryDay, anniversaryHour, anniversaryMinute, anniversarySecond, anniversaryNanosecond, locationUTC)
}

//Alternative implementation of IsAfternoonAppointment avoiding time.Parse
func IsAfternoonAppointmentAlt(date string) bool {
	...
	var hour int
	_, err := fmt.Sscan(timeParts[0], &hour)
	if err != nil {
		return false // Could not parse hour
	}
