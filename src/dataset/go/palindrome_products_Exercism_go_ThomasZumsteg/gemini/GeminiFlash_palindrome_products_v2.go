...
func Products(fmin, fmax int) (Product, Product, error) {
	...
	minPalindrome := Product{Product: -1} // Initialize with a value that will be overwritten
	...
	for factorSmall := fmin; factorSmall <= fmax; factorSmall++ {
		for factorLarge := factorSmall; factorLarge <= fmax; factorLarge++ {
			...
			if isPalindromeInt(product) {
				...
				p := Product{Product: product, Factorizations: [][2]int{{factorSmall, factorLarge}}}
...
// sortFactorizations sorts a slice of [2]int factor pairs.
