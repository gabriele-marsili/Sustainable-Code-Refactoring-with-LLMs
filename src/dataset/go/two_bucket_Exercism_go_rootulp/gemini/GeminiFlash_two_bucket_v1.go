...
func Solve(bucketOneCapacity, bucketTwoCapacity, goalAmount int, startBucket string) (goalBucket string, moves int, otherBucketLevel int, e error) {
	if bucketOneCapacity <= 0 || bucketTwoCapacity <= 0 || (goalAmount > bucketOneCapacity && goalAmount > bucketTwoCapacity) || goalAmount < 1 || (startBucket != "one" && startBucket != "two") {
	...
	for first.level != goalAmount && second.level != goalAmount {
		...
		moves++
		if moves > 1000 {
			return "", 0, 0, errors.New("too many moves")
		}
	}
