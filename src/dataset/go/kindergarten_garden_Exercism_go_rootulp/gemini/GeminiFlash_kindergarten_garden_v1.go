...
func NewGarden(diagram string, children []string) (*Garden, error) {
	if !strings.HasPrefix(diagram, "\n") {
		return nil, fmt.Errorf("invalid diagram: missing newline prefix")
	}

	rows := getDiagramRows(diagram)
	if len(rows) != 2 {
		return nil, fmt.Errorf("invalid diagram: must have two rows")
	}

	if len(rows[0]) != len(rows[1]) {
		return nil, fmt.Errorf("invalid diagram: rows must have equal length")
	}

	if len(rows[0])%2 != 0 {
		return nil, fmt.Errorf("invalid diagram: row length must be even")
	}

	for _, row := range rows {
		for _, cupCode := range row {
			if _, ok := cupCodeToPlant[cupCode]; !ok {
				return nil, fmt.Errorf("invalid diagram: invalid cup code")
			}
		}
	}
