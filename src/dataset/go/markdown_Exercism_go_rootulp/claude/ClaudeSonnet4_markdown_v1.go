...
func Render(markdown string) (html string) {
	...
	return handleUnorderedList(builder.String())
}
...
func handleListItem(line string) string {
	if len(line) < 2 {
		return fmt.Sprintf("<li>%s</li>", line)
	}
...
func handleItalic(markdown string) string {
	...
	if secondPos < len(markdown)-1 && markdown[secondPos+1] == '_' {
		return markdown
	}
	
	return markdown[:first] + "<em>" + markdown[first+1:secondPos] + "</em>" + markdown[secondPos+1:]
}
