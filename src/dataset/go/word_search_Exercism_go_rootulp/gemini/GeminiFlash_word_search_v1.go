...
func (g Grid) Search(word string) ([2][2]int, bool) {
	...
	for row_i := 0; row_i < g.rows; row_i++ {
		for col_i := 0; col_i < len(g.grid[row_i]); col_i++ {
...
func (g Grid) isMatch(word string, row int, col int, direction string, wordLen int) bool {
	for i := 0; i < wordLen; i++ {
		...
		switch direction {
		case NORTH:
			char, ok = g.SafeGet(row-i, col)
		case EAST:
			char, ok = g.SafeGet(row, col+i)
		case SOUTH:
			char, ok = g.SafeGet(row+i, col)
		case WEST:
			char, ok = g.SafeGet(row, col-i)
		case NORTH_EAST:
			char, ok = g.SafeGet(row-i, col+i)
		case NORTH_WEST:
			char, ok = g.SafeGet(row-i, col-i)
		case SOUTH_EAST:
			char, ok = g.SafeGet(row+i, col+i)
		case SOUTH_WEST:
			char, ok = g.SafeGet(row+i, col-i)
		default:
			return false // Should not happen, but handle invalid direction
		}
