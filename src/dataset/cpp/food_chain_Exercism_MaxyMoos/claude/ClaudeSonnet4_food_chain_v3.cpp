...
// ANIMAL UTIL CLASS
////////////////////////
animal::animal(string name, string secondLine) : name(std::move(name)), secondLine(std::move(secondLine)), optional(this->name)
{
}

animal::animal(string name, string secondLine, string optional) : name(std::move(name)), secondLine(std::move(secondLine))
{
    this->optional = optional.empty() ? this->name : std::move(optional);
}
...
{
    const auto& current_animal = animals[index-1];
    string result;
    result.reserve(256);
...
{
    string result;
    result.reserve(1024 * (highIndex - lowIndex + 1));
