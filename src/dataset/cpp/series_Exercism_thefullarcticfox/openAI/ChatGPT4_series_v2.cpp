...
namespace series {
	std::vector<int> digits(const std::string& number) {
		std::vector<int> res;
		res.reserve(number.size()); // Reserve memory upfront to avoid reallocations
	...
	std::vector<std::vector<int>> slice(const std::string& number, unsigned int digitcount) {
		...
		res.reserve(number.size() - digitcount + 1); // Reserve memory upfront to avoid reallocations
		for (size_t digit = 0; digit + digitcount <= number.size(); ++digit) {
			...
			res.push_back(std::move(slice)); // Use move semantics to avoid copying
		}
		return res;
	}
}  // namespace series