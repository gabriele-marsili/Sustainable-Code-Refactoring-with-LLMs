...
namespace beer {
    ...
    inline std::string verse(int verse_num) {
        if (verse_num == 0) return zero_bottles;
        if (verse_num == 1) return one_bottle;

        return std::to_string(verse_num) + " bottles of beer on the wall, " + std::to_string(verse_num) +
               " bottles of beer.\nTake one down and pass it around, " +
               std::to_string(verse_num - 1) + ((verse_num - 1 == 1) ? " bottle" : " bottles") +
               " of beer on the wall.\n";
    }

    std::string sing(int from) {
        std::string result;
        result.reserve(300 * from); // Pre-allocate memory to reduce reallocations
    ...
    std::string sing(int from, int to) {
        std::string result;
        result.reserve(300 * (from - to + 1)); // Pre-allocate memory to reduce reallocations
