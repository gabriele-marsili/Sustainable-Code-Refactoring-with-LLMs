...
namespace queen_attack {
    ...
    bool chess_board::can_attack() const {
        ...
        // Check diagonal attack using absolute difference
        int row_diff = white_queen.first - black_queen.first;
        int col_diff = white_queen.second - black_queen.second;
        return (row_diff * row_diff) == (col_diff * col_diff);
    }

    chess_board::operator std::string() const {
        string result;
        result.reserve(120); // Pre-allocate: 8*8 chars + 7*8 spaces + 8 newlines
