...
BOOST_AUTO_TEST_CASE(time_tests) {
  for (timeTest t : timeCases) {
    ...
    BOOST_REQUIRE_MESSAGE(t.expected == actual,
                          errorMsg(t.expected, actual, t.msg));
  }
}
...
BOOST_AUTO_TEST_CASE(add_tests) {
  for (addTest a : addCases) {
    ...
    BOOST_REQUIRE_MESSAGE(a.expected == actual,
                          errorMsg(a.expected, actual, a.msg));
  }
}
  ...
  for (equalTest e : equalCases) {
    ...
    if (e.expected)
      BOOST_REQUIRE_MESSAGE(clock1 == clock2,
                            errorMsg(string(clock1), string(clock2), e.msg));
    else
      BOOST_REQUIRE_MESSAGE(clock1 != clock2, "[" << string(clock1)
                                                  << " == " << string(clock2)
                                                  << "] test case: " << e.msg);
  }
}
