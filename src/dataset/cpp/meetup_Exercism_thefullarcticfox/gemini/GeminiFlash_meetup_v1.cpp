...
namespace meetup {
	...
	date	scheduler::teenth(wd_enum weekday) const {
		date day(_year, _month, 13); // Start at 13th, closer to the target
		int day_of_week = day.day_of_week().as_enum();

		int diff = weekday - day_of_week;
		if (diff < 0) {
			diff += 7;
		}
	...
	date	scheduler::nthwday(wd_enum weekday, int n, bool fromEnd) const {
		...
		if (day.month() != _month) {
			// If the calculated date is outside the month, adjust back.  This handles edge cases and avoids potential infinite loops.
