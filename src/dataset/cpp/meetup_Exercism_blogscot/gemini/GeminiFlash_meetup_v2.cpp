...
int scheduler::find_teenth(scheduler_day day) const {
  for (auto pos = 13; pos <= 19; ++pos) {
    if (month_days[pos] == day) {
      return pos;
    }
  }
  return -1; // Should not happen, but good to have a return in case of error.
}
...
int scheduler::find_nth(scheduler_day day, int nth) const {
  ...
  return -1; // Or throw an exception, as the nth day wasn't found.
}
...
int scheduler::find_last(scheduler_day day) const {
  ...
  return -1; // Or throw an exception, as the last day wasn't found.
}
...
scheduler::scheduler(months_of_year month, uint16_t year)
  ...
  month_days.resize(days_in_month + 1);
