...
namespace meetup
{
	...
	date scheduler::teenth(boost::date_time::weekdays day) const
	{
		...
		boost::date_time::weekdays current_day = d.day_of_week();
		int days_to_add = (day - current_day + 7) % 7;
	...
	date scheduler::nth(boost::date_time::weekdays day, int n) const
	{
		...
		boost::date_time::weekdays first_day_of_week = first_of_month.day_of_week();
		int days_to_first_occurrence = (day - first_day_of_week + 7) % 7;
		return first_of_month + days(days_to_first_occurrence + (n - 1) * 7);
	}

	date scheduler::last(boost::date_time::weekdays day) const
	{
		int last_day_of_month = boost::gregorian::gregorian_calendar::end_of_month_day(year, month);
		date last_day(year, month, last_day_of_month);
		boost::date_time::weekdays last_day_of_week = last_day.day_of_week();
		int days_to_subtract = (last_day_of_week - day + 7) % 7;
