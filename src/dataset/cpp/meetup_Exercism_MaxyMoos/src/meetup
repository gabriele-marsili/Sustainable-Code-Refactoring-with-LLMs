...
namespace meetup {
    ...
    date scheduler::teenth_generic(boost::date_time::weekdays day) const {
        int offset = (day - date(refYear, refMonth, 13).day_of_week() + 7) % 7;
        return date(refYear, refMonth, 13 + offset);
    }

    date scheduler::nth_generic(int n, boost::date_time::weekdays day) const {
        date first_day(refYear, refMonth, 1);
        int offset = (day - first_day.day_of_week() + 7) % 7;
        return first_day + date_duration(offset + (n - 1) * 7);
    }

    date scheduler::last_generic(boost::date_time::weekdays day) const {
        date last_day = date(refYear, refMonth, 1).end_of_month();
        int offset = (last_day.day_of_week() - day + 7) % 7;
        return last_day - date_duration(offset);
    }
