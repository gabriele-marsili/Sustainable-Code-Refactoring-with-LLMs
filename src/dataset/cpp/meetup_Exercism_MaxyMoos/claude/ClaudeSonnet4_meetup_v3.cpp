...
namespace meetup {
    ...
    date scheduler::teenth_generic(boost::date_time::weekdays day) const
    {
        ...
        int days_to_add = (day + 7 - tmp.day_of_week()) % 7;
        return tmp + date_duration(days_to_add);
    }
    ...
    date scheduler::nth_generic(int n, boost::date_time::weekdays day) const
    {
        ...
        int days_to_add = (day + 7 - tmp.day_of_week()) % 7;
        tmp += date_duration(days_to_add);
        return tmp + date_duration(7 * (n - 1));
    }

    date scheduler::last_generic(boost::date_time::weekdays day) const
    {
        date tmp = date(refYear, refMonth, 1).end_of_month();
        int days_to_subtract = (tmp.day_of_week() + 7 - day) % 7;
        return tmp - date_duration(days_to_subtract);
    }
