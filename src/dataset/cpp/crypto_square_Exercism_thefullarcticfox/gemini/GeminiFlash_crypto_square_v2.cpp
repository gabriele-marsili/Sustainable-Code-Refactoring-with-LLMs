...
std::vector<std::string> make_str_segments(const std::string& str,
  ...
  if (invert_cols) cols = (str.size() + cols - 1) / cols;
...
std::string cipher::cipher_text() const {
  ...
  size_t rows = (normalized_text.size() + cols - 1) / cols;
  std::string res;
  ...
  for (size_t i = 0; i < cols; ++i) {
    for (size_t j = 0; j < rows; ++j) {
      ...
      if (index < normalized_text.size()) {
        res.push_back(normalized_text[index]);
      }
    }
  }
  return res;
...
std::string cipher::normalized_cipher_text() const {
  ...
  std::string res;
  res.reserve(rows * (cols + 1)); // Pre-allocate space for segments and spaces
