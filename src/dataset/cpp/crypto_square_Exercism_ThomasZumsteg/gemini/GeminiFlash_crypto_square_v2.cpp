...
// Optimized function to add two vectors in-place (vec1 += vec2)
...
// Optimized function to multiply a vector by a scalar in-place
void multiply_vector(vector<double>& vec, double scalar) {
    for (double& val : vec) {
        val *= scalar;
    }
}

// Example usage (demonstrates the functions, but not part of the optimization)
int main() {
