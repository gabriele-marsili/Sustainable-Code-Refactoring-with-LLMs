...
int find_maximum(const vector<int>& nums) {
    ...
    int max_val = nums[0];
    for (size_t i = 1; i < nums.size(); ++i) { // Use size_t for indexing
        if (nums[i] > max_val) {
            max_val = nums[i];
        }
    }
...
// Optimized function to calculate the product of two numbers
long long calculate_product(int a, int b) {
    return static_cast<long long>(a) * b; // Avoid potential integer overflow
}
