...
namespace crypto_square
{
    ...
    {
        std::string result;
        result.reserve(initial.size()); // Reserve memory to avoid reallocations
    ...
    const std::vector<std::string> cipher::create_segments(std::string text)
    {
        ...
        segments.reserve((text.length() + size - 1) / size); // Preallocate memory
    ...
    std::string cipher::cipher_text()
    {
        ...
        std::string result;
        result.reserve(rows * columns); // Reserve memory to avoid reallocations
    ...
    std::string cipher::normalized_cipher_text()
    {
        ...
        std::string result;
        result.reserve(segments.size() * (segments[0].length() + 1) - 1); // Reserve memory
