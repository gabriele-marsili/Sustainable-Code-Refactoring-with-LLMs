#include <iostream>
#include <vector>
#include <algorithm>

using namespace std;

// Optimized function to calculate the sum of even numbers in a vector
long long sum_of_even_numbers(const vector<int>& numbers) {
    long long sum = 0;
    for (int number : numbers) {
        if ((number & 1) == 0) { // Bitwise AND is faster than modulo for checking even numbers
            sum += number;
        }
    }
    return sum;
}

// Optimized function to find the maximum element in a vector
int find_maximum(const vector<int>& numbers) {
    if (numbers.empty()) {
        return -1; // Or throw an exception, depending on desired behavior for empty input
    }
    int max_element = numbers[0];
    for (size_t i = 1; i < numbers.size(); ++i) {
        if (numbers[i] > max_element) {
            max_element = numbers[i];
        }
    }
    return max_element;
}

// Optimized function to filter positive numbers from a vector
vector<int> filter_positive_numbers(const vector<int>& numbers) {
    vector<int> positive_numbers;
    positive_numbers.reserve(numbers.size()); // Reserve space to avoid reallocations
    for (int number : numbers) {
        if (number > 0) {
            positive_numbers.push_back(number);
        }
    }
    return positive_numbers;
}

// Optimized function to check if a number is prime
bool is_prime(int number) {
    if (number <= 1) return false;
    if (number <= 3) return true;
    if (number % 2 == 0 || number % 3 == 0) return false;
    for (int i = 5; i * i <= number; i = i + 6)
        if (number % i == 0 || number % (i + 2) == 0)
            return false;
    return true;
}

// Optimized function to count prime numbers in a vector
int count_prime_numbers(const vector<int>& numbers) {
    int count = 0;
    for (int number : numbers) {
        if (is_prime(number)) {
            count++;
        }
    }
    return count;
}

// Optimized function to reverse a string
string reverse_string(const string& str) {
    string reversed_str = str;
    reverse(reversed_str.begin(), reversed_str.end());
    return reversed_str;
}

// Optimized function to calculate the factorial of a number
long long factorial(int n) {
    if (n < 0) return -1; // Or throw an exception, depending on desired behavior for negative input
    if (n == 0) return 1;
    long long result = 1;
    for (int i = 1; i <= n; ++i) {
        result *= i;
    }
    return result;
}

// Optimized function to calculate the power of a number
double power(double base, int exponent) {
    if (exponent == 0) return 1;
    double result = 1;
    if (exponent > 0) {
        for (int i = 0; i < exponent; ++i) {
            result *= base;
        }
    } else {
        for (int i = 0; i < -exponent; ++i) {
            result /= base;
        }
    }
    return result;
}

// Optimized function to calculate the sum of numbers in a range
long long sum_of_range(int start, int end) {
    if (start > end) return 0; // Or handle the error as appropriate
    return static_cast<long long>(end - start + 1) * (start + end) / 2; // Use formula for sum of arithmetic series
}

// Optimized function to find the first occurrence of a character in a string
int find_first_occurrence(const string& str, char character) {
    for (size_t i = 0; i < str.length(); ++i) {
        if (str[i] == character) {
            return static_cast<int>(i);
        }
    }
    return -1; // Return -1 if not found
}