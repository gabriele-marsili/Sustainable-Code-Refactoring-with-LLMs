...
def find_first_missing_2(a):
    ...
    # Move all non-positive numbers and numbers > n to the end of the array
    j = 0
    for i in range(n):
        if 0 < a[i] <= n:
            a[i], a[j] = a[j], a[i]
    ...
    # Consider only the first 'j' elements of the array
    for i in range(j):
        index = abs(a[i]) - 1
        if index < j and a[index] > 0:
            a[index] = -a[index]

    # Find the first positive element in the modified array
    for i in range(j):
        if a[i] > 0:
            return i + 1
