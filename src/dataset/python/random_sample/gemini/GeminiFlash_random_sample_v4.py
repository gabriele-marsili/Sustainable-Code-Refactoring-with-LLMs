...
def reservoir_sampling(arr, k):
    """
    Selects a random sample of k items from arr using reservoir sampling.
    """
    if k <= 0:
        return []
    if k >= len(arr):
        return arr[:]  # Return a copy to avoid modifying the original array

    sample = arr[:k]  # Initialize reservoir
...
def probabilistic_sampling(arr, k):
    """
    Selects a random sample of k items from arr using probabilistic sampling.
    """
    if k <= 0:
        return []
    if k >= len(arr):
    ...
    for i, el in enumerate(arr):
        if random.random() < (k / n):
            ...
            k -= 1
            if k == 0:
                break # Optimization: Stop early if we have enough elements
        n -= 1
    return sample