...
class Solution:
    ...
    def _generate(self, start: int, end: int) -> List[TreeNode]:
        ...
        if key in self._memo:
            return [self._clone_tree(tree, start) for tree in self._memo[key]]
        ...
        normalized_result = [self._clone_tree(tree, -start + 1) for tree in result]
        self._memo[key] = normalized_result
        return result
