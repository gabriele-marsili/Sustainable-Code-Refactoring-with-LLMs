...
const getNumericKeys = (object: Record<number, unknown>): number[] =>
    Object.keys(object).map(Number);

export default function generate({ minFactor = 1, maxFactor = Number.MAX_VALUE }) {
    const factorsByProduct: Record<number, Array<[number, number]>> = {};
    ...
    const getOutputFactors = (factors: Array<[number, number]>): [number, number] | Array<[number, number]> =>
