...
class Pangram {
    ...
    isPangram() {
        let seenMask = 0;
        const length = __classPrivateFieldGet(this, _Pangram_input, "f").length;
        for (let i = 0; i < length; i++) {
            const c = __classPrivateFieldGet(this, _Pangram_input, "f").charCodeAt(i);
            if (c >= 65 && c <= 90) {
                seenMask |= 1 << (c - 65);
            }
            else if (c >= 97 && c <= 122) {
