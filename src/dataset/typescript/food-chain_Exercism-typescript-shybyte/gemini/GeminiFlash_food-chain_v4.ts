...
const firstLineInVerse = (animal: string): string => `I know an old lady who swallowed a ${animal}.`;

const buildVerse = (verseNumber: number): string => {
    if (verseNumber === 8) {
...
const verse = (verseNumber: number): string => buildVerse(verseNumber);

const verses = (minVerse: number = 1, maxVerse: number = 8): string => {
    let result = '';
