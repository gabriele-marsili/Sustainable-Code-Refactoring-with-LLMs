...
export class WordSearch {
    ...
    public find(words: string[]): WordsType {
        ...
        const isValid = (p: Point) =>
            p[0] >= 0 && p[0] < matrix.length && p[1] >= 0 && p[1] < matrix[p[0]].length;
            ...
            for (let c = 0; c < matrix.length; c++) {
                for (let r = 0; r < matrix[c].length; r++) {
                    ...
                    for (const [dx, dy] of vectors) {
                        let i = 1;
                        let currentPoint: Point = [c, r];

                        while (i < wordLength) {
                            currentPoint = [currentPoint[0] + dx, currentPoint[1] + dy];
                            if (!isValid(currentPoint) || matrix[currentPoint[0]][currentPoint[1]] !== word[i]) break;
                            i++;
                        }
