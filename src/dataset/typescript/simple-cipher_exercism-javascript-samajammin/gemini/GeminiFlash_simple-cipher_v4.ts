class SimpleCipher {
  ...
  private genKey(): string {
    ...
    for (let i = 0; i < keyLength; i++) {
      key += this.alphabet[Math.floor(Math.random() * this.alphabetLength)];
    }
    return key;
  }
  ...
  private intToChar(int: number): string {
    let index: number = int % this.alphabetLength;
    if (index < 0) {
      index += this.alphabetLength;
    }
  ...
  private convert(str: string, isEncode: boolean): string {
    ...
    for (let i = 0; i < str.length; i++) {
      ...
      const shiftedInt = isEncode ? charInt + keyCharInt : charInt - keyCharInt;
      result += this.intToChar(shiftedInt);
    }
    return result;
  }
