class SimpleCipher {
  ...
  private genKey(): string {
    let result = '';
    for (let i = 0; i < 100; i++) {
      result += SimpleCipher.alphabet[Math.floor(Math.random() * SimpleCipher.alphabetLength)];
    }
    return result;
  }
  ...
  private intToChar(int: number): string {
    ...
    return SimpleCipher.alphabet[index];
  }
  ...
  private convert(str: string, isEncode: boolean): string {
    ...
    for (let i = 0; i < str.length; i++) {
      ...
      let newCharInt: number;
      if (isEncode) {
        newCharInt = charInt + keyCharInt;
      } else {
        newCharInt = charInt - keyCharInt;
      }
