...
const callbackObservers = new Set<Observer<any>>();
...
function createInput<T>(
  ...
  const equalFn = equal === true ? strictEqual : 
                  typeof equal === 'function' ? equal : null;
  ...
  const write: SetterFn<T> = (newValue) => {
    ...
    if (activeObserver) {
      const before = activeObserver.updateFn(activeObserver.value);
      const after = activeObserver.updateFn(activeObserver.value);
      
      if (before !== after && callbackObservers.size > 0) {
        for (const observer of callbackObservers) {
          observer.updateFn(observer.value);
        }
      }
    } else if (callbackObservers.size > 0) {
      for (const observer of callbackObservers) {
        observer.updateFn(observer.value);
      }
    }
...
function createComputed<T>(
  ...
  const prevActiveObserver = activeObserver;
  activeObserver = observer;
  
  const getter = (): T => {
    if (isStale) {
      const newValue = updateFn(cachedValue);
...
function createCallback<T>(updateFn: UpdateFn<T>, value?: T): UnsubscribeFn {
  const callback: Observer<T> = {
    name: `cb_${Date.now()}_${Math.floor(Math.random() * 1000)}`,
    value,
    updateFn,
  };
