...
class Board {
    ...
    canConnectX() {
        ...
        return this.canConnect('X', Array.from({ length: this.content.length }, (_, rowIndex) => [rowIndex, -1]), isRightBorder);
    }
    canConnectO() {
        const rows = this.content.length;
        const isBottomBorder = ([row, _col]) => row === rows;
        return this.canConnect('O', Array.from({ length: this.content[0].length }, (_, colIndex) => [-1, colIndex]), isBottomBorder);
    }
