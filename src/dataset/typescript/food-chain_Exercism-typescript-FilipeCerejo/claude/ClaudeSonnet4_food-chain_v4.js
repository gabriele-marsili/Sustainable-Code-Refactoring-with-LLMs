...
const SPIDER_VERSE = ' that wriggled and jiggled and tickled inside her.';
const FLY_VERSE = `I don't know why she swallowed the fly. Perhaps she'll die.\n`;
const DEATH_VERSE = `She's dead, of course!\n`;
function verse(v) {
    const animalIndex = v - 1;
    const animal = animals[animalIndex];
    const parts = [`I know an old lady who swallowed a ${animal.name}.\n`];
    if (v === ANIMALS_LENGTH) {
        parts.push(DEATH_VERSE);
        return parts.join('');
    }
    ...
    for (let i = maxIndex; i > 0; i--) {
        ...
        const suffix = i === 2 ? SPIDER_VERSE : '.';
        parts.push(`She swallowed the ${currentAnimal.name} to catch the ${previousAnimal.name}${suffix}\n`);
    }
    parts.push(FLY_VERSE);
    return parts.join('');
}
