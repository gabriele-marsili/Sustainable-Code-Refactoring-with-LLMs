...
function verse(v) {
    ...
    if (v === animals.length) {
        ...
        if (animal.secondVerse)
            lines.push(animal.secondVerse);
        for (let i = v - 1; i > 0; i--) {
            const current = animals[i];
            const previous = animals[i - 1];
            lines.push(`She swallowed the ${current.name} to catch the ${previous.name}${i - 1 === 1 ? ' that wriggled and jiggled and tickled inside her.' : '.'}`);
        }
