...
const toRoman = (decimal) => {
    ...
    if (length > 3)
        roman.push('M'.repeat(Number(stringed[length - 4])));
    if (length > 2)
        roman.push(digitLetters('C', 'D', 'M', Number(stringed[length - 3])));
    if (length > 1)
        roman.push(digitLetters('X', 'L', 'C', Number(stringed[length - 2])));
