...
class SimpleCipher {
    ...
    transcode(text, type) {
        ...
        let result = '';
        const processedText = text.replace(/\W/g, '').toLowerCase();
        const textLength = processedText.length;
        for (let i = 0; i < textLength; i++) {
            const char = processedText[i];
            const charCode = char.charCodeAt(0);
            const keyCharCode = this.cipherKey[i % keyLength].charCodeAt(0);
            result += String.fromCharCode(this.modulo(charCode - MIN_CHAR_CODE + (keyCharCode - MIN_CHAR_CODE) * multiplier, OFFSET + 1) + MIN_CHAR_CODE);
        }
        return result;
    }
    encode(text) {
