...
public class Crypto {
    ...
    public int getSquareSize() {
        if (squareSize == -1) {
            ...
            squareSize = (int) Math.ceil(Math.sqrt(length));
        }
        return squareSize;
    }
    ...
    public List<String> getPlaintextSegments() {
        ...
        List<String> segments = new ArrayList<>((normalizedPlaintext.length() + size - 1) / size);
        for (int i = 0; i < normalizedPlaintext.length(); i += size) {
            segments.add(normalizedPlaintext.substring(i, Math.min(i + size, normalizedPlaintext.length())));
        }
        return segments;
    }
