class SpaceAge {
    ...
    double onMercury() {
        return seconds / (MERCURY_YEARS * EARTH_YEAR_SECONDS);
    }

    double onVenus() {
        return seconds / (VENUS_YEARS * EARTH_YEAR_SECONDS);
    }

    double onMars() {
        return seconds / (MARS_YEARS * EARTH_YEAR_SECONDS);
    }

    double onJupiter() {
        return seconds / (JUPITER_YEARS * EARTH_YEAR_SECONDS);
    }

    double onSaturn() {
        return seconds / (SATURN_YEARS * EARTH_YEAR_SECONDS);
    }

    double onUranus() {
        return seconds / (URANUS_YEARS * EARTH_YEAR_SECONDS);
    }

    double onNeptune() {
        return seconds / (NEPTUNE_YEARS * EARTH_YEAR_SECONDS);
    }
}