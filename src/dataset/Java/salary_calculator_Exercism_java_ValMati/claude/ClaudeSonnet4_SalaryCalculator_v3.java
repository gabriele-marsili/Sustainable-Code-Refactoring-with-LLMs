public class SalaryCalculator {
    ...
    public double finalSalary(int daysSkipped, int productsSold) {
        double baseSalaryAdjusted = BASE_SALARY * (daysSkipped > MAX_SKIPPED_DAYS_WITHOUT_PENALTY ? SALARY_MULTIPLIER_WITH_PENALTY : 1.0);
        double bonus = productsSold * (productsSold >= MIN_SOLD_FOR_EXTRA_BONUS ? EXTRA_BONUS : BASIC_BONUS);
        double calculatedSalary = baseSalaryAdjusted + bonus;
