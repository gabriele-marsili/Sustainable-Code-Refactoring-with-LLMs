public class SalaryCalculator {
    ...
    public double bonusForProductsSold(int productsSold) {
        int multiplier = productsSold >= MIN_SOLD_FOR_EXTRA_BONUS ? EXTRA_BONUS : BASIC_BONUS;
        return productsSold * multiplier;
    }

    public double finalSalary(int daysSkipped, int productsSold) {
        double salaryMultiplier = daysSkipped > MAX_SKIPPED_DAYS_WITHOUT_PENALTY ? SALARY_MULTIPLIER_WITH_PENALTY : 1.0;
        int bonusMultiplier = productsSold >= MIN_SOLD_FOR_EXTRA_BONUS ? EXTRA_BONUS : BASIC_BONUS;
        
        double calculatedSalary = (BASE_SALARY * salaryMultiplier) + (productsSold * bonusMultiplier);
