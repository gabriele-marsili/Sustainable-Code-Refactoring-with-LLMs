class BeerSong {
    ...
    private static final String WALL_END = " of beer on the wall.\n\n";
    ...
    public String sing(int count, int repeat) {
        final var song = new StringBuilder(repeat * 150); // Pre-allocate capacity
        while (repeat-- > 0) {
            if (count == 0) {
                song.append(NO_MORE_START)
                    .append(BOTTLE_CACHE[INITIAL_BOTTLES])
                    .append(WALL_END);
                count = INITIAL_BOTTLES;
            } else {
                String currentBottle = BOTTLE_CACHE[count];
                song.append(currentBottle)
                    .append(BEER_ON_WALL)
                    .append(currentBottle)
                    .append(BEER_NEWLINE)
                    .append(count == 1 ? TAKE_IT : TAKE_ONE);
                --count;
                song.append(BOTTLE_CACHE[count])
                    .append(WALL_END);
            }
        }
