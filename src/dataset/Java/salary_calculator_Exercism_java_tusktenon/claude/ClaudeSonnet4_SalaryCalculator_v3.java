public class SalaryCalculator {
  ...
  private static final double SALARY_MULTIPLIER_PENALTY = 0.85; // 1 - 0.15
  ...
  public double salaryMultiplier(int daysSkipped) {
    return daysSkipped >= DAYS_SKIPPED_ALLOWANCE ? SALARY_MULTIPLIER_PENALTY : 1.0;
  }
  ...
  public double finalSalary(int daysSkipped, int productsSold) {
    final double salaryMultiplier = daysSkipped >= DAYS_SKIPPED_ALLOWANCE ? SALARY_MULTIPLIER_PENALTY : 1.0;
    final int bonusMultiplier = productsSold >= IMPROVED_BONUS_MINIMUM_PRODUCTS
        ? IMPROVED_BONUS_MULTIPLIER
        : BASE_BONUS_MULTIPLIER;
    
    final double calculatedSalary = BASE_SALARY * salaryMultiplier + productsSold * bonusMultiplier;
