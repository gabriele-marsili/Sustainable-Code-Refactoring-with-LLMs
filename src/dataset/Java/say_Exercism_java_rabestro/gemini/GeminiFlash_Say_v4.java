public final class Say {
    ...
    private String sayNumber(long number) {
        if (number < 20) {
            ...
            return TENS[ten] + "-" + LESS_THAN_20[one];
        } else if (number < 1000) {
            return sayMagnitude(number, 100, HUNDRED);
        } else if (number < 1_000_000) {
            return sayMagnitude(number, 1_000, THOUSAND);
        } else if (number < 1_000_000_000) {
            return sayMagnitude(number, 1_000_000, MILLION);
        } else {
            return sayMagnitude(number, 1_000_000_000, BILLION);
        }
    }
