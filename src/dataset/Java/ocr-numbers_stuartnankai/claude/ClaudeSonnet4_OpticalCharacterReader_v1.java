...
public class OpticalCharacterReader {
    ...
    // Pre-initialized static map to avoid recreation
    ...
    String parse(List arrayList) throws IllegalArgumentException {
        ...
        String firstRow = arrayList.get(0).toString();
        ...
        for (int y = 0; y < countRow; y++) {
            ...
            for (int i = 0; i < countCol; i++) {
                ...
                for (int j = y * 4; j < y * 4 + 4; j++) {
                    ...
                    pattern.append(arrayList.get(j).toString().substring(i * 3, i * 3 + 3));
                }
                result.append(checkMap(pattern.toString()));
            }
        }
    ...
    private String checkMap(String pattern) {
        return NUMBER_MAPPING.getOrDefault(pattern, "?");
    }
}