...
public class KillerSudokuHelper {
    ...
    private static class CombinationGenerator {
        ...
        private CombinationGenerator(Collection<Integer> excluded) {
        ...
        List<List<Integer>> findCombinations(int sum, int size) {
            ...
            findCombinationsRecursive(1, sum, size, new ArrayList<>(), results);
            return results;
        }

        private void findCombinationsRecursive(int currentStartDigit, int remainingSum, int remainingSize, List<Integer> currentCombination, List<List<Integer>> results) {
            if (remainingSize == 0) {
                if (remainingSum == 0) {
            ...
            for (int digit = currentStartDigit; digit <= maxPossibleFirstDigit; digit++) {
                if (!excludedDigits[digit]) {
                    ...
                    findCombinationsRecursive(digit + 1, remainingSum - digit, remainingSize - 1, currentCombination, results);
