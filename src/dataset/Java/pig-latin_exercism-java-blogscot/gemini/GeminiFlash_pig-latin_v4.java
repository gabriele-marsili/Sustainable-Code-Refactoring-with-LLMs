...
class PigLatinTranslator {
  ...
  private String translateWord(String word) {
    ...
    if (isVowel(firstChar)) {
      if (firstChar == 'u' && len > 1 && word.charAt(len > 1 ? 1 : 0) == 'q') return word + "ay";
      return word + "ay";
    }

    if (len >= 3 && (word.startsWith("thr") || word.startsWith("sch"))) return word.substring(3) + word.substring(0, 3) + "ay";
    if (len >= 3 && !isVowel(firstChar) && word.substring(1, 3).startsWith("qu")) return word.substring(3) + word.substring(0, 3) + "ay";
