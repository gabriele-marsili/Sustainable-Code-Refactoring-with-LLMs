...
RobotGridStatus_t robot_init(void) {
    return (RobotGridStatus_t){ .grid = {0, 0}, .bearing = Heading_North };
}
    ...
    while (*c) {
        switch(*c) {
            ...
            case Command_Advance:
                robot->grid.x_position += (robot->bearing == Heading_East) - (robot->bearing == Heading_West);
                robot->grid.y_position += (robot->bearing == Heading_North) - (robot->bearing == Heading_South);
                break;
        }
        c++;
    }
