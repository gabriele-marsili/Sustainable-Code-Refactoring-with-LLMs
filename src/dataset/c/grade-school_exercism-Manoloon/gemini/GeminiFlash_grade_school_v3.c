...
bool add_student(roster_t *roster, char *name, uint8_t grade) {
    if (roster == NULL || name == NULL || *name == '\0' || roster->count >= MAX_STUDENTS) return false;
    ...
    while (pos < roster->count) {
        if (grade < roster->students[pos].grade) break;
        if (grade == roster->students[pos].grade && strcmp(name, roster->students[pos].name) < 0) break;
        pos++;
    }
