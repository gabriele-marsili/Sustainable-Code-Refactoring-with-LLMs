...
{
   struct list* _list = (struct list*)calloc(1, sizeof(struct list));
   return _list;
}
...
{
   struct list_node* new_first = (struct list_node*)malloc(sizeof(struct list_node));
   if(!new_first) return;
