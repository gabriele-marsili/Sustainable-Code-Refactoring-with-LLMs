...
struct list *list_create(void)
{
   ...
   _list->first = NULL;
   _list->last = NULL;
   _list->count = 0; // Initialize count to 0
...
{
   return list->count; // Return the precomputed count
}
...
{
   if(list->last == NULL) return -1; // List is empty
...
{
   if(list->first == NULL) return -1; // List is empty
