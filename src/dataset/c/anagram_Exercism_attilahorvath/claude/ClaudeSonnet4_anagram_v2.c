...
static int code_point_count(const char *str, const char *cp, int str_len) {
    int count = 0;
    int bytes = code_point_bytes(cp[0]);
    
    for (int i = 0; i <= str_len - bytes; i++) {
        if (memcmp(str + i, cp, bytes) == 0) {
            count++;
        }
    }
    return count;
...
static void downcase(char *dst, const char *src) {
    int i = 0;
    char c;
...
int is_anagram(char *a, char *b) {
    ...
    int i = 0;
    while (i < len_a) {
        int bytes = code_point_bytes(aBuf[i]);
        if (bytes <= 0) return 0;
        
        if (code_point_count(aBuf, aBuf + i, len_a) != 
            code_point_count(bBuf, aBuf + i, len_b)) {
            return 0;
        }
...
struct Vector anagrams_for(char *word, struct Vector candidates) {
    ...
    int size = 0;
    int limit = (candidates.size < MAX_VEC_SIZE) ? candidates.size : MAX_VEC_SIZE;
