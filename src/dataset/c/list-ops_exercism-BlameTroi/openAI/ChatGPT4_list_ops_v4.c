...
list_t *filter_list(list_t *list, bool (*filter)(list_element_t)) {
  ...
  list_element_t *temp = malloc(list->length * sizeof(list_element_t));
  if (!temp) return NULL;
  for (size_t i = 0; i < list->length; i++) {
    if (filter(list->elements[i])) {
      temp[filteredCount++] = list->elements[i];
    }
  }
  ...
  memcpy(filtered->elements, temp, filteredCount * sizeof(list_element_t));
