...
struct cell *create_compute1_cell(struct reactor *r, struct cell *c,
				  compute1 fun)
{
	struct cell *res = create_input_cell(r, fun(get_cell_value(c)));
	res->type = ONE_VAR;
	res->fun1 = fun;
	res->dep_a = c;
...
{
	struct cell *res = create_input_cell(r, fun(get_cell_value(ca), get_cell_value(cb)));
	res->type = TWO_VARS;
	res->fun2 = fun;
	res->dep_a = ca;
	res->dep_b = cb;
...
void remove_callback(struct cell *c, callback_id id)
{
	...
	c->clb[id] = NULL;
	c->clb_obj[id] = NULL; //clear object pointer as well
}
