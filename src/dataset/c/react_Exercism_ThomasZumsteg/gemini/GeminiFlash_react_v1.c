...
struct cell *create_cell(struct reactor *r, void update(struct cell *),
    ...
    if (r->n_cells >= MAX_CELLS) {
        fprintf(stderr, "Error: Maximum number of cells reached.\n");
        return NULL;
    }
...
struct cell *create_compute1_cell(struct reactor *r, struct cell *input,
    ...
    int initial_value = func(get_cell_value(input));
    struct cell *c = create_cell(r, update_compute1, (void *)func, initial_value);
    if (!c) return NULL;
...
struct cell *create_compute2_cell(struct reactor *r, struct cell *input1,
    ...
    int initial_value = func(get_cell_value(input1), get_cell_value(input2));
    struct cell *c = create_cell(r, update_compute2, (void *)func, initial_value);
    if (!c) return NULL;
