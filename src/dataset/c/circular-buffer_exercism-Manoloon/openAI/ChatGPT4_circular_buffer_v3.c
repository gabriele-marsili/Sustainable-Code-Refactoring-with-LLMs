...
circular_buffer_t *new_circular_buffer(size_t capacity)
{
    ...
    new_buffer->values = (buffer_value_t*)calloc(capacity, sizeof(buffer_value_t));
...
{
    if (!buffer || buffer->num_items >= buffer->capacity)
    {
        errno = ENOBUFS;
        return EXIT_FAILURE;
    }
...
int16_t overwrite(circular_buffer_t *buffer, buffer_value_t values)
{
    ...
    {
        buffer->values[buffer->head] = values;
        buffer->head = (buffer->head + 1) % buffer->capacity;
    }
    else
    {
