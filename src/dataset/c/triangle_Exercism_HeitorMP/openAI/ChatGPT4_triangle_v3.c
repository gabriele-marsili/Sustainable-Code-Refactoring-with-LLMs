...
static inline int is_nonzero_and_valid(triangle_t sides) {
...
int is_valid_triangle(triangle_t sides) {
    return is_nonzero_and_valid(sides);
}

int is_equilateral(triangle_t sides) {
    return is_nonzero_and_valid(sides) && (sides.a == sides.b && sides.b == sides.c);
}
