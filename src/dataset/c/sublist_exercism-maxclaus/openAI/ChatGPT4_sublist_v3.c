...
comparison_result_t check_lists_v1(int *list_to_compare, int *base_list,
  ...
  for (size_t i = 0; i <= list_to_compare_element_count - base_list_element_count; i++) {
    size_t j = 0;
    while (j < base_list_element_count && list_to_compare[i + j] == base_list[j]) {
      j++;
    }
...
comparison_result_t check_lists_v2(int *list_to_compare, int *base_list,
  ...
  size_t smaller_count = sublist ? list_to_compare_element_count : base_list_element_count;
  size_t greater_count = sublist ? base_list_element_count : list_to_compare_element_count;

  for (size_t i = 0; i <= greater_count - smaller_count; i++) {
    if (memcmp(smaller_list, &greater_list[i], smaller_count * sizeof(int)) == 0) {
