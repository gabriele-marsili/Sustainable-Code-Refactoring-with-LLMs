...
{
	return (robot_grid_status_t){
		(bearing >= 0 && bearing < HEADING_MAX) ? (bearing_t)bearing : (bearing_t)DEFAULT_BEARING,
		{x, y}
	};
}
...
void robot_advance(robot_grid_status_t *robot)
{
	...
	case HEADING_NORTH: robot->grid.y_position++; break;
	case HEADING_EAST:  robot->grid.x_position++; break;
	case HEADING_SOUTH: robot->grid.y_position--; break;
	case HEADING_WEST:  robot->grid.x_position--; break;
	default: break;
	}
}
...
void robot_simulator(robot_grid_status_t *robot, const char *commands)
{
	...
	for (; *commands; commands++) {
		switch (*commands) {
		case COMMAND_LEFT:    robot_turn_left(robot); break;
		case COMMAND_RIGHT:   robot_turn_right(robot); break;
		case COMMAND_ADVANCE: robot_advance(robot); break;
		default: break;
		}
	}
}