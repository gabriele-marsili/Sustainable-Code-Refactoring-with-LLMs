...
static void free_factor_list(struct factors *p);
    ...
    for (int i = from; i <= to; i++) {
        for (int k = i; k <= to; k++) {
            ...
            if (is_palindrome(n)) {
                if (n < res->smallest) {
                    res->smallest = n;
                    free_factor_list(res->factors_sm);
                    res->factors_sm = NULL;
                    ...
                    res->largest = n;
                    free_factor_list(res->factors_lg);
                    res->factors_lg = NULL;
...
{
    if (!p) return;
    free_factor_list(p->factors_lg);
    free_factor_list(p->factors_sm);
    free(p);
}
