...
class StatsHandler:
    ...
    def load_output_files(self):
        ...
        if not self.output_directory.exists():
            raise FileNotFoundError(
                f"[ERRORE] Directory non trovata: {self.output_directory}"
            )

        json_files = list(self.output_directory.glob("*.json"))
        if not json_files:
            raise FileNotFoundError(
                f"[ERRORE] Nessun file .json trovato in: {self.output_directory}"
            )
        ...
        if self.cluster_filter:
            ...
            if not json_files:
                raise FileNotFoundError(
                    f"[ERRORE] Nessun file trovato per il cluster: {self.cluster_filter}"
                )

        print(
            f"Trovati {len(json_files)} file JSON per il cluster '{self.cluster_filter or 'tutti'}'"
        )
        ...
        for cluster_name, runs in self.baseline_data.items():
            ...
            for run_number, run_data in runs.items():
                ...
                for language, entries in results.items():
                    ...
                    for entry in entries:
                        baseline_data["execution_times"].append(
            ...
            if baseline_data["execution_times"]:
                self.baseline_stats[cluster_name] = {
                    ...
                    "std_execution_time": stdev(baseline_data["execution_times"])
                    if len(baseline_data["execution_times"]) > 1
                    else 0,
                    "avg_cpu_usage": mean(baseline_data["cpu_usages"]),
                    "std_cpu_usage": stdev(baseline_data["cpu_usages"])
                    if len(baseline_data["cpu_usages"]) > 1
                    else 0,
                    "avg_ram_usage": mean(baseline_data["ram_usages"]),
                    "std_ram_usage": stdev(baseline_data["ram_usages"])
                    if len(baseline_data["ram_usages"]) > 1
                    else 0,
        ...
        for cluster_name, versions in self.data.items():
            ...
            for version, runs in versions.items():
                    ...
                    for language, entries in results.items():
                        ...
                        for entry in entries:
                            ...
                            for llm_result in llm_results:
                                ...
                                if test_passed:
                                    version_data["execution_times"].append(
                                        llm_result.get("execution_time_ms", 0)
                                    )
                                    version_data["cpu_usages"].append(
                                        llm_result.get("CPU_usage", 0)
                                    )
                                    version_data["ram_usages"].append(
                                        llm_result.get("RAM_usage", 0)
                                    )

                                version_data["regression_tests_passed"].append(
                                ...
                                version_data["llm_results"].append(
                                    {
                                        ...
                                        "execution_time_ms": llm_result.get(
                                            "execution_time_ms", 0
                                        ),
                ...
                if version_data["execution_times"]:
                    cpu_usages_clean = [
                        x for x in version_data["cpu_usages"] if x is not None
                    ]
                    ram_usages_clean = [
                        x for x in version_data["ram_usages"] if x is not None
                    ]
                    exec_times_clean = [
                        x for x in version_data["execution_times"] if x is not None
                    ]

                    stats = {
                        "avg_execution_time": mean(exec_times_clean),
                        "std_execution_time": stdev(exec_times_clean)
                        if len(exec_times_clean) > 1
                        else 0,
                        "avg_cpu_usage": mean(cpu_usages_clean)
                        if cpu_usages_clean
                        else 0,
                        "std_cpu_usage": stdev(cpu_usages_clean)
                        if len(cpu_usages_clean) > 1
                        else 0,
                        "avg_ram_usage": mean(ram_usages_clean)
                        if ram_usages_clean
                        else 0,
                        "std_ram_usage": stdev(ram_usages_clean)
                        if len(ram_usages_clean) > 1
                        else 0,
                        "total_tests": sum(version_data["regression_tests_total"]),
                        "tests_passed": sum(version_data["regression_tests_passed"]),
                        "pass_rate": (
                            sum(version_data["regression_tests_passed"])
                            / sum(version_data["regression_tests_total"])
                            * 100
                        )
                        if sum(version_data["regression_tests_total"]) > 0
                        else 0,
                    ...
                    self.version_stats[version]["execution_times"].extend(
                        version_data["execution_times"]
                    )
                    self.version_stats[version]["cpu_usages"].extend(
                        version_data["cpu_usages"]
                    )
                    self.version_stats[version]["ram_usages"].extend(
                        version_data["ram_usages"]
                    )
                    self.version_stats[version]["pass_rates"].append(stats["pass_rate"])
                    self.version_stats[version]["clusters"].append(cluster_name)
                    self.version_stats[version]["total_entries"] += stats[
                        "total_entries"
                    ]
        ...
        for version, data in self.version_stats.items():
            if data["execution_times"]:
                ...
                self.version_comparison[version] = {
                    "avg_execution_time": safe_mean(data["execution_times"]),
                    "avg_cpu_usage": safe_mean(data["cpu_usages"]),
                    "avg_ram_usage": safe_mean(data["ram_usages"]),
                    "avg_pass_rate": safe_mean(data["pass_rates"]),
                    "total_entries": data["total_entries"],
                    "cluster_count": len(set(data["clusters"])),
                }

    def plot_single_metric(
        self, metric_name, metric_key, unit="", include_baseline=True
    ):
        ...
        # Plot delle versioni
        bars = ax.bar(
            versions, values, color="lightblue", alpha=0.7, label="Prompt Versions"
        )
            ...
            if baseline_values:
                ...
                for bar, value in zip(baseline_bars, baseline_values):
                    ax.text(
                        ...
                        bar.get_height() + (max(values + baseline_values) * 0.01),
                        f"{value:.1f}",
                        ha="center",
                        va="bottom",
                    )
        ...
        ax.set_title(
            f"{metric_name} Comparison - Cluster: {self.cluster_filter or 'All'}"
        )
    ...
    def plot_all_metrics_separately(self):
        ...
        for metric_name, metric_key, unit in metrics:
            include_baseline = (
                metric_key != "avg_pass_rate"
            )  # Baseline non ha pass rate
    ...
    def find_best_prompt_version(self):
        ...
        for version, stats in self.version_comparison.items():
            ...
            pass_rate_penalty = (
                100 - stats["avg_pass_rate"]
            ) * 10  # Penalità per test falliti
    ...
    def print_version_statistics(self):
        ...
        if self.baseline_vs_versions_comparison:
            ...
            for cluster, comparison in self.baseline_vs_versions_comparison.items():
                ...
                print(
                    f"  Baseline: {baseline['avg_execution_time']:.2f}ms, "
                ...
                for version, stats in comparison["versions"].items():
                    ...
                    exec_diff = (
                        (stats["avg_execution_time"] - baseline["avg_execution_time"])
                        / baseline["avg_execution_time"]
                    ) * 100
                    cpu_diff = (
                        (stats["avg_cpu_usage"] - baseline["avg_cpu_usage"])
                        / baseline["avg_cpu_usage"]
                    ) * 100
                    ram_diff = (
                        (stats["avg_ram_usage"] - baseline["avg_ram_usage"])
                        / baseline["avg_ram_usage"]
                    ) * 100

                    print(
                        f"  {version}: {stats['avg_execution_time']:.2f}ms ({exec_diff:+.1f}%), "
    ...
    def plot_version_comparison(self):
        ...
        for i, (metric, name) in enumerate(zip(metrics, metric_names)):
            ...
            bars = ax.bar(
                versions,
                values,
                color="lightblue" if metric != "avg_pass_rate" else "lightgreen",
            )
    ...
    def plot_cluster_version_heatmap(self):
        ...
        for i, (metric, name) in enumerate(zip(metrics, metric_names)):
            ...
            for ci, cluster in enumerate(clusters):
                for vi, version in enumerate(versions):
                    if version in self.cluster_version_stats[cluster]:
                        matrix[ci, vi] = self.cluster_version_stats[cluster][version][
                            metric
                        ]
            ...
            im = ax.imshow(
                matrix,
                aspect="auto",
                cmap="RdYlGn_r" if metric != "pass_rate" else "RdYlGn",
            )
    ...
    def compute_regression_test_statistics(self):
        ...
        regression_per_lang = defaultdict(
            lambda: defaultdict(lambda: {"passed": 0, "failed": 0, "total": 0})
        )
    ...
    def print_dataset_statistics(self):
        ...
        clusters_same_language = defaultdict(
        ...
        if data:
            for key in data:
                ...
                for entry in language_entries:
                    not_llm_entry_counter += 1
                    quantity = len(
                        [x for x in entry["LLMs"] if f"_v{version}" in x["filename"]]
                    )
...
def count(handler: StatsHandler):
    for version in range(1, 5):
        ...
        (llm_entry_quantity, not_llm_entry_quantity) = handler.count_entries_cluster(
            f_name, version
        )
        print(
            f"quanitity of entries for file {f_name} : {not_llm_entry_quantity} | llm entries v {version} : {llm_entry_quantity}"
        )

        for run in range(1, 6):
            f_name = f"leap_results_v{version}_{run}.json"
            (llm_entry_quantity, not_llm_entry_quantity) = (
                handler.count_entries_result_file(f_name)
            )
            print(
                f"quanitity of entries for file {f_name} : {not_llm_entry_quantity} | llm entries : {llm_entry_quantity}"
            )
...
if __name__ == "__main__":
    ...
    handler = StatsHandler(
        str(utility_paths.OUTPUT_DIR_FILEPATH), cluster_filter=cluster_name
    )
