...
class ImprovedPerformanceAnalyzer:
    ...
    def calculate_comparison_statistics(self) -> Dict[str, Any]:
        """
        Build comparison statistics between base and LLM groups using per-entry means.
        Returns a dictionary with overall reduction percentages and group-wise reductions:
        - overall: reductions for cpu/ram/time comparing base overall vs llm overall
        - by_model: reductions comparing base overall vs each model's overall
        - by_prompt_version: reductions comparing base overall vs each prompt version overall
        - by_language: reductions comparing base language means vs llm language means (aligned by language)
        - by_model_prompt: reductions comparing base overall vs each model+prompt overall
        """
        stats = {
            "overall": {},
            "by_model": {},
        ...
        if base_overall and llm_overall:
            for metric in ["cpu_mean", "ram_mean", "time_mean"]:
                stats["overall"][metric.replace("_mean","_reduction_pct")] = self.calculate_improvement(base_overall[metric], llm_overall[metric])
        # by_model
        for model, data in self.llm_statistics.get("by_model", {}).items():
            stats["by_model"][model] = {}
            for metric in ["cpu_mean", "ram_mean", "time_mean"]:
                stats["by_model"][model][metric.replace("_mean","_reduction_pct")] = self.calculate_improvement(base_overall.get(metric, 0.0), data[metric])
        # by_prompt_version
        for pv, data in self.llm_statistics.get("by_prompt_version", {}).items():
            stats["by_prompt_version"][pv] = {}
            for metric in ["cpu_mean", "ram_mean", "time_mean"]:
                stats["by_prompt_version"][pv][metric.replace("_mean","_reduction_pct")] = self.calculate_improvement(base_overall.get(metric, 0.0), data[metric])
        # by_language: align languages present in both
        base_lang = self.base_statistics.get("by_language", {})
        llm_lang = self.llm_statistics.get("by_language", {})
        for lang, bstats in base_lang.items():
            if lang in llm_lang:
                stats["by_language"][lang] = {}
                for metric in ["cpu_mean", "ram_mean", "time_mean"]:
                    stats["by_language"][lang][metric.replace("_mean","_reduction_pct")] = self.calculate_improvement(bstats[metric], llm_lang[lang][metric])
        # by_model_prompt
        for mp, data in self.llm_statistics.get("by_model_prompt", {}).items():
            stats["by_model_prompt"][mp] = {}
            for metric in ["cpu_mean", "ram_mean", "time_mean"]:
                stats["by_model_prompt"][mp][metric.replace("_mean","_reduction_pct")] = self.calculate_improvement(base_overall.get(metric, 0.0), data[metric])
        return stats
    ...
    def create_comparison_plots(self) -> None:
        ...
        for i, (metric, title) in enumerate(zip(metrics, titles)):
            ...
            # Calculate and show reduction percentage (lower is better). Display with negative sign.
            reduction = self.calculate_improvement(base_overall[metric], llm_overall[metric])
            color = 'green' if reduction > 0 else 'red'
            axes[i].text(0.5, 0.95, f'Reduction: {-reduction:.1f}%', 
    ...
    def save_all_results(self) -> None:
        ...
        results = {
            ...
            "comparison_statistics": getattr(self, 'comparison_statistics', {})
        }
    ...
    def run_complete_analysis(self) -> None:
        ...
        # Comparison statistics (base vs LLM groups)
        self.comparison_statistics = self.calculate_comparison_statistics()
        ...
        self.create_model_prompt_comparison_plots()
        self.create_language_comparison_plots()
        self.create_model_comparison_plots()
        self.create_prompt_comparison_plots()
    ...
    def print_analysis_summary(self) -> None:
        ...
        print("  • Comparisons show absolute mean values per metric")
        print("  • Lower bars for CPU/RAM/Time indicate better performance")
        print("="*100)

    def create_model_prompt_comparison_plots(self) -> None:
        """
        Create per-metric bar charts comparing Base overall vs each Model+Prompt version overall.
        Uses per-entry means (5 runs) aggregated to overall means for each group.
        """
        if not self.base_statistics or not self.llm_statistics:
            return
        base_overall = self.base_statistics.get("overall", {})
        by_model_prompt = self.llm_statistics.get("by_model_prompt", {})
        if not base_overall or not by_model_prompt:
            return
        # Prepare categories
        categories = ["Base"] + list(by_model_prompt.keys())
        metric_keys = [
            ("cpu_mean", "CPU Usage (mean)", "06_base_vs_model_prompt_cpu.png"),
            ("ram_mean", "RAM Usage (mean)", "06_base_vs_model_prompt_ram.png"),
            ("time_mean", "Execution Time (mean)", "06_base_vs_model_prompt_time.png"),
            ("test_pass_rate", "Test Pass Rate", "06_base_vs_model_prompt_test.png"),
        ]
        for metric, title, fname in metric_keys:
            values = [base_overall.get(metric, 0.0)] + [by_model_prompt[k][metric] for k in by_model_prompt.keys()]
            base_val = values[0]
            plt.figure(figsize=(max(10, len(categories)*0.9), 6))
            bars = plt.bar(categories, values, alpha=0.8)
            plt.xticks(rotation=45)
            plt.ylabel("Value")
            plt.title(f"Base vs Model+Prompt: {title} (with % reduction)\n(per-entry means, 5 runs)")
            for idx, (b, v) in enumerate(zip(bars, values)):
                plt.text(b.get_x()+b.get_width()/2., v*1.01 if v>=0 else v*0.99, f"{v:.2f}", ha='center', va='bottom' if v>=0 else 'top', fontsize=9)
                if idx > 0 and base_val > 0:
                    reduction = ((base_val - v) / max(base_val, EPSILON)) * 100.0
                    plt.text(b.get_x()+b.get_width()/2., (v + base_val)/2, f"{-reduction:.1f}%", ha='center', va='bottom', fontsize=9, color=('green' if reduction>0 else 'red'))
            plt.tight_layout()
            plt.savefig(self.reports_dir / fname, dpi=300, bbox_inches='tight')
            plt.close()

    def create_language_comparison_plots(self) -> None:
        """Compare Base vs LLM by language per metric, with % reduction labels."""
        if not self.base_statistics or not self.llm_statistics:
            return
        base_lang = self.base_statistics.get('by_language', {})
        llm_lang = self.llm_statistics.get('by_language', {})
        common_langs = [lang for lang in base_lang.keys() if lang in llm_lang]
        if not common_langs:
            return
        metric_keys = [
            ("cpu_mean", "CPU Usage (mean)", "07_base_vs_llm_by_language_cpu.png"),
            ("ram_mean", "RAM Usage (mean)", "07_base_vs_llm_by_language_ram.png"),
            ("time_mean", "Execution Time (mean)", "07_base_vs_llm_by_language_time.png"),
        ]
        for metric, title, fname in metric_keys:
            base_vals = [base_lang[lang][metric] for lang in common_langs]
            llm_vals = [llm_lang[lang][metric] for lang in common_langs]
            x = np.arange(len(common_langs))
            width = 0.4
            plt.figure(figsize=(max(10, len(common_langs)*0.9), 6))
            b1 = plt.bar(x - width/2, base_vals, width, label='Base')
            b2 = plt.bar(x + width/2, llm_vals, width, label='LLM')
            plt.xticks(x, common_langs, rotation=45)
            plt.ylabel('Value')
            plt.title(f'Base vs LLM by Language: {title}\n(per-entry means, 5 runs)')
            plt.legend()
            for bx, bv, lv in zip(b2, base_vals, llm_vals):
                red = ((bv - lv) / max(bv, EPSILON)) * 100.0 if bv > 0 else 0.0
                plt.text(bx.get_x()+bx.get_width()/2., lv + (bv-lv)/2 if bv!=lv else lv*1.02, f"{-red:.1f}%", ha='center', va='bottom', fontsize=9, color=('green' if red>0 else 'red'))
            plt.tight_layout()
            plt.savefig(self.reports_dir / fname, dpi=300, bbox_inches='tight')
            plt.close()

    def create_model_comparison_plots(self) -> None:
        """Compare Base vs each Model (overall) per metric with % reductions."""
        if not self.base_statistics or not self.llm_statistics:
            return
        base_overall = self.base_statistics.get('overall', {})
        by_model = self.llm_statistics.get('by_model', {})
        if not base_overall or not by_model:
            return
        categories = ['Base'] + list(by_model.keys())
        metric_keys = [
            ("cpu_mean", "CPU Usage (mean)", "06b_base_vs_model_cpu.png"),
            ("ram_mean", "RAM Usage (mean)", "06b_base_vs_model_ram.png"),
            ("time_mean", "Execution Time (mean)", "06b_base_vs_model_time.png"),
        ]
        for metric, title, fname in metric_keys:
            values = [base_overall.get(metric, 0.0)] + [by_model[k][metric] for k in by_model.keys()]
            base_val = values[0]
            plt.figure(figsize=(max(10, len(categories)*0.9), 6))
            bars = plt.bar(categories, values, alpha=0.8)
            plt.xticks(rotation=45)
            plt.ylabel('Value')
            plt.title(f'Base vs Model: {title} (with % reduction)\n(per-entry means, 5 runs)')
            for idx, (b, v) in enumerate(zip(bars, values)):
                plt.text(b.get_x()+b.get_width()/2., v*1.01 if v>=0 else v*0.99, f"{v:.2f}", ha='center', va='bottom' if v>=0 else 'top', fontsize=9)
                if idx > 0 and base_val > 0:
                    reduction = ((base_val - v) / max(base_val, EPSILON)) * 100.0
    ...
    def create_prompt_comparison_plots(self) -> None:
        """Compare Base vs each Prompt Version (overall across models) per metric with % reductions."""
        if not self.base_statistics or not self.llm_statistics:
            return
        base_overall = self.base_statistics.get('overall', {})
        by_prompt = self.llm_statistics.get('by_prompt_version', {})
        if not base_overall or not by_prompt:
            return
        categories = ['Base'] + list(by_prompt.keys())
        metric_keys = [
            ("cpu_mean", "CPU Usage (mean)", "06c_base_vs_prompt_cpu.png"),
            ("ram_mean", "RAM Usage (mean)", "06c_base_vs_prompt_ram.png"),
            ("time_mean", "Execution Time (mean)", "06c_base_vs_prompt_time.png"),
        ]
        for metric, title, fname in metric_keys:
            values = [base_overall.get(metric, 0.0)] + [by_prompt[k][metric] for k in by_prompt.keys()]
            base_val = values[0]
            plt.figure(figsize=(max(10, len(categories)*0.9), 6))
            bars = plt.bar(categories, values, alpha=0.8)
            plt.xticks(rotation=45)
            plt.ylabel('Value')
            plt.title(f'Base vs Prompt Version: {title} (with % reduction)\n(per-entry means, 5 runs)')
            for idx, (b, v) in enumerate(zip(bars, values)):
                plt.text(b.get_x()+b.get_width()/2., v*1.01 if v>=0 else v*0.99, f"{v:.2f}", ha='center', va='bottom' if v>=0 else 'top', fontsize=9)
                if idx > 0 and base_val > 0:
                    reduction = ((base_val - v) / max(base_val, EPSILON)) * 100.0
